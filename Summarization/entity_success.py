...
def set_args():
    ...
    parser.add_argument("--mode", dest="mode", type=str,
                        default="single_entity_test",
                        choices=["single_entity_test", "oracle_add_test", "oracle_drop_test", "oracle", "k_entity_test",
                                 "interactive"])
...
def eval_ctrlsum(model, data, ents, idx_to_example, logger, test_ents=None):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(data), batch_size):  # function as batch iterator
            ...
            res = model.sample(batch_inputs, beam=4, prefix_tokens=None, lenpen=1.0, max_len_b=140, min_len=1,
                               no_repeat_ngram_size=3, extra_gen_cls_kwargs=None)
            allypred.extend(res)
    ...
    for j, (pred, ents) in enumerate(zip(allypred, all_ents)):
        ...
        if args.mode == 'single_entity_test':
            ...
            condition = all([ent.lower() in pred.lower() for ent in cur_oracle_ents]) and (
                        to_add_ent.lower() in pred.lower())
...
def eval(model, valid_dataset, scaler, logger, args, tokenizer, nlp, idx_to_example, seed=0, test_ents=None):
    ...
    valid_dataloader = get_dataloader(
        tokenizer, args.num_workers_summary, valid_dataset, args.valid_size_per_gpu_summary, args.max_length,
    ...
    for j in range(len(allytrue)):
        ...
        if args.mode == 'single_entity_test':
            ...
            cur_oracle_ents = test_ents[j]['oracle']
            to_add_ent = test_ents[j]['add']
            condition = all([ent.lower() in summary.lower() for ent in cur_oracle_ents]) and (
                        to_add_ent.lower() in summary.lower())
        success_list.append(condition)
        if condition:
            successes = successes + 1
            example_success[example_id].append(1)
    ...
    if args.mode == 'single_entity_test':
        for (idx, all_ents) in enumerate([full_ents, lead3_ents, other_ents]):
            ...
            if args.model == 'CTRLsum_origin':
                ...
                example_successes, successes, inputs0, summaries0, all_ents0 = eval_ctrlsum(
                    model, new_data, new_allents, idx_to_example, logger
                )
                allsec += successes
                allnum += len(new_data)
            else:
                ...
                inputs0, all_ents0, labels0, summaries0, successes, num, example_successes = eval(
                    model, new_valid_dataset, scaler, logger, args, tokenizer, nlp, idx_to_example
                )
                allsec += successes
                allnum += num
            ...
            logger.info(
                f"success by examples, examples #{len(example_success_rates)}, mean: {np.mean(example_success_rates)}, \
        ...
        for (idx, all_ents) in enumerate([full_ents]):
            ...
            if args.model == 'CTRLsum_origin':
                logger.info(f'start CTRLsum_origin eval')
                example_successes, successes, inputs0, summaries0, all_ents0, suc_list = eval_ctrlsum(
                    model, new_data,new_allents,idx_to_example,logger,test_ents=ents_to_pass
                )
                allsec += successes
                allnum += len(new_data)
            else:
                ...
                new_valid_dataset.allent = new_allents
                new_valid_dataset.num_entries = len(new_data)
                inputs0, all_ents0, labels0, summaries0, successes, num, example_successes, suc_list = eval(
                    model, new_valid_dataset,scaler,logger,args,tokenizer,nlp,idx_to_example
                )
                allsec += successes
                allnum += num
        ...
        if args.model == 'CTRLsum_origin':
            ...
            new_valid_dataset.num_entries = len(new_data)
            inputs0, all_ents0, labels0, summaries0, successes, num, example_successes, suc_list = eval(
                model,new_valid_dataset,scaler, logger,args, tokenizer,nlp,idx_to_example,test_ents=ents_to_pass
            )
            allsec += successes
            allnum += num
        ...
        with torch.no_grad():
            while True:
                ...
                # process data
                input_res = valid_dataset.tokenizer.batch_encode_plus([inp], padding=False,
                                                                      max_length=valid_dataset.maxlen, truncation=True,
                                                                      return_tensors="pt")
                input_ids = input_res['input_ids']
                input_attn_mask = input_res['attention_mask']

                target_res = valid_dataset.tokenizer.batch_encode_plus([tar], padding=False,
                                                                       max_length=valid_dataset.maxlen, truncation=True,
                                                                       return_tensors="pt")
                target_ids = target_res['input_ids']
                target_attn_mask = target_res['attention_mask']

                ent_res = valid_dataset.tokenizer.batch_encode_plus([ent_chain], padding=False,
                                                                    max_length=valid_dataset.maxlen, truncation=True,
                                                                    return_tensors="pt")
                ...
                if "CTRLsum" in args.model:
                    ...
                    preds = model.sample([new_text], beam=4, prefix_tokens=None, lenpen=1.0, max_len_b=140, min_len=1,
                                         no_repeat_ngram_size=3, extra_gen_cls_kwargs=None)
                else:
                    sen, target, preds = model._generative_step(inputs)
