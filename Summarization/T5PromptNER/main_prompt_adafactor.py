            ...
            if scaler is not None:
                ...
                for ii in range(sennum):
                    ...
                    for j in range(len(allentintar)):
                        if allentintar[j] in alleninpred:
                            hasentnum += 1
            for aa in range(len(target)):
                print(target[aa],"\t*****************\t",preds[aa])
    f1score = float(hasentnum) / float(allentnum)
...
def train(args, model, train_dataset,valid_dataset,test_dataset):
    ...
    base_optimizer_arguments = {"lr": args.lr, "clip_threshold":args.max_grad_norm, "decay_rate":-0.8, "weight_decay": args.weight_decay,
