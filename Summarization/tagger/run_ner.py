...
def main():
    ...
    # Prepare model
    config = BertConfig.from_pretrained(args.bert_model, num_labels=num_labels, finetuning_task=args.task_name,  cache_dir=args.cache_dir)
    ...
    if args.do_train:
        ...
        for i in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if i >= 0 and args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                if thisevalscore > bestevalscore:
                    logger.info('----best result, save one model----')
        ...
        model = Ner.from_pretrained(args.output_dir)
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        # logger.info('we just use the pre-trained checkpoint.')
