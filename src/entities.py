...
def spacy_ents_stats(data, spacy_nlp):
    text_words_n = []
    text_ents_n = []
    text_ents_n_unique = []
    summary_words_n = []
    ...
    for idx in tqdm(range(int(len(data) / 1))):
        # text
        ...
        # summary
        summary_data = data[idx]['highlights']
...
def spacy_build_ents_frequency(data, spacy_nlp):
    ents_freq = {}
    for idx in tqdm(range(int(len(data) / 100))):
        text_data = data[idx]['article']
        ents = spacy_nlp(text_data).ents
        for x in ents:
            ent = x.text
